...
import {
  isManagedBrowserActLauncher,
  runOysterBrowserAction,
} from "../src/product/browser-act.js";
...
describe.skipIf(process.platform === "win32")(
  "BrowserAct browser provider wrapper",
  () => {
...
describe.runIf(process.platform === "win32")(
  "BrowserAct Windows PowerShell runner",
  () => {
    it("preserves PowerShell launcher arguments containing special characters", async () => {
      const root = await mkdtemp(join(tmpdir(), "oyster-browser-act-win-"));
      const commandPath = join(root, "browser-act.ps1");
      const callsPath = join(root, "calls.txt");
      const previousCommand = process.env.OYSTER_BROWSER_ACT_COMMAND;
      await writeFile(
        commandPath,
        `$args | Set-Content -LiteralPath '${callsPath.replaceAll("'", "''")}' -Encoding UTF8
Write-Output "ok"
exit 0
`,
        "utf8",
      ...
      try {
        const script = "document.title + '%PATH%' + '&' + '中文'";
        await expect(
          runOysterBrowserAction("eval", {
            session: "windows-argument-check",
            script,
          }),
        ).resolves.toMatchObject({ ok: true });
        await expect(readFile(callsPath, "utf8")).resolves.toContain(script);
      } finally {
        restoreEnv("OYSTER_BROWSER_ACT_COMMAND", previousCommand);
        await rm(root, { recursive: true, force: true });
      }
    });

    it("recognizes packaged Windows PowerShell launchers as managed", () => {
      expect(
        isManagedBrowserActLauncher(
          String.raw`C:\Program Files\OysterWorkflow\resources\bin\oysterworkflow-browseract.ps1`,
        ),
      ).toBe(true);
    });
  },
