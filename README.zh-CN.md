...
OysterWorkflow 源码已在本仓库公开，并采用混合许可模式：

- 开放核心使用 [Apache License 2.0](./LICENSE)。除非某个路径被明确列为 PolyForm 范围，本仓库中的 OysterWorkflow 原创代码默认使用 Apache-2.0。
- [LICENSING.md](./LICENSING.md) 列出的桌面产品代码与资产使用 [PolyForm Noncommercial 1.0.0](./legal/PolyForm-Noncommercial-1.0.0.md)。可以依照该许可证进行非商业使用；商业使用需要另行获得 OysterWorkflow 授权。
- 第三方组件继续遵守各自的许可证，详情见 [THIRD-PARTY-NOTICES.md](./THIRD-PARTY-NOTICES.md)。

GitHub 会根据仓库根目录的许可证显示 Apache-2.0。复用桌面产品目录中的文件前，请同时查看距离该文件最近的 `LICENSING.md`。
...
欢迎提交问题、文档改进和 Pull Request。贡献代码前请先阅读[贡献指南](./CONTRIBUTING.md)和[许可范围说明](./LICENSING.md)，不要在公开 issue 或提交中包含凭证、客户数据、私有链接或未经脱敏的工作流证据。
